...
public class BookController {
    ...
    private Long nextId = 1L; // Initialize the next available ID
    ...
    @PostMapping("/remove")
    public String removeBooks(@RequestParam(value = "bookIds", required = false) Long[] bookIds) {
        if (bookIds != null) {
            for (Long id : bookIds) {
    ...
    public String addComment(@PathVariable Long id, @RequestParam("newComment") String newComment) {
        Book book = findBookById(id);
        if (book != null) {
            List<String> comments = book.getComments();
            if (comments == null) {
    ...
    public String editCommentForm(@PathVariable Long id, @PathVariable int commentIndex, Model model) {
        Book book = findBookById(id);
    ...
    public String editCommentSubmit(@PathVariable Long id, @PathVariable int commentIndex, @RequestParam("editedComment") String editedComment) {
        Book book = findBookById(id);
        if (book != null && book.getComments() != null && commentIndex >= 0 && commentIndex < book.getComments().size()) {
    ...
    public String deleteComment(@PathVariable Long id, @PathVariable int commentIndex) {
        Book book = findBookById(id);
        if (book != null && book.getComments() != null && commentIndex >= 0 && commentIndex < book.getComments().size()) {
    ...
    public String addToCart(@PathVariable Long id) {
        ...
        if (bookToAdd != null) {
            ...
            for (Book cartBook : shoppingCart) {
                if (cartBook.getId().equals(bookToAdd.getId())) {
                    // Update the quantity
